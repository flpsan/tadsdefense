...
public class GerenteBatalha {
    ...
    public void update() {
        if (entidade.getPropriedades().podeAtacar()) {
            ...
            if (isAtacando()) {
                podeAtacar = Mapa.podeAtacar(entidade, atacando) || (ehConstrucao && entidadesAVista.contains(atacando) && ((Construcao) entidade).temCondicaoDeAtacar());
                if (podeAtacar && roundsCont++ > roundsToAttack) {
                    ...
                    entidade.goTo(Mapa.getCelulaMaisProxima(entidade, atacando));
                }
            }
    ...
    public int getLadoAlvo() {
        return Mapa.viradoPara(entidade, atacando);
    }
